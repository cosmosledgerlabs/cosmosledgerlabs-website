...
## 11. Production QA Revision Round (mobile / WeChat / Samsung)

**Root cause of most mobile issues:** the stylesheet had no `text-size-adjust`, so WeChat / Samsung / Android in-app browsers auto-inflated text — overflowing the funding badge (Samsung, image 1) and overlapping the Roadmap title with its status badge (WeChat, images 3–4). Fixed globally.

| # | Item | Status | What changed |
|---|------|--------|--------------|
| 1 | Responsive / no horizontal scroll / overflow | ✅ Fixed | Added `text-size-adjust:100%`, `overflow-x:hidden` (html+body), global `overflow-wrap:break-word`. Verified **0px horizontal overflow** at 360/390/768/1440. |
| 2 | WeChat built-in browser | ✅ Root-caused | Same `text-size-adjust` fix (the standard remedy for WeChat/Android font-boosting) + removed WebGL (see #4) which is the main source of in-app-browser jank/CLS. |
| 3 | Technical Roadmap cards equal + no overlap | ✅ Fixed | Badges fixed uniform width (88px desktop / 108px mobile). On mobile the title/badge now **stack vertically** — badge can no longer overlap the title. Verified overlap=false at all widths. |
| 4 | Hero blue effect (remove particles) | ✅ Replaced | Removed the entire three.js particle/voxel shader; replaced with a **smooth CSS neon-blue radial glow** (two layered gradients, gentle pulse, honors reduced-motion). No particles/noise. Also removed 1.3 MB `three` dependency + vendor file → lighter, faster, and far better in-app-browser compatibility. |
| 5 | Aladdin Strategic Partner | ⏸ **BLOCKED — needs your OK** | You previously said "do NOT publish until contract signed + Peter's wording approved." Not added. Confirm it's signed/approved and I'll add it. |
| 6 | Investor Deck button | ⏸ **Needs target** | Happy to re-add left of "Request Full Deck", aligned/uniform — but it needs somewhere to point (public deck PDF upload / email / disabled) or it 404s again. |
| 7 | Two paragraphs wrapping/margins | ✅ Fixed | `text-size-adjust` + `overflow-wrap` fixes the mid-word email break and the oversized WeChat text; monospace paragraphs now wrap normally and align across breakpoints. |
| 8 | Delete demo-video placeholder | ✅ Removed | Removed the `// VIDEO PLACEHOLDER … demo-video.mp4` block and its now-dead CSS. |
| 9 | Bottom circle decoration | ❓ **Need you to mark it** | Could not find a matching element in code. Please circle it in a screenshot (like image 1) so I remove the right thing. |
| 10 | Font brightness / contrast | ✅ Improved | Brightened the too-dim "SCALE" title and PLANNED/FUTURE badges (were failing WCAG AA and near-invisible on mobile). Consistent weight retained; glow not increased (kept readable). |
| 11 | WeChat display (images 3–7) | ✅ Addressed | Covered by #1–#4, #7. |
| 12 | Samsung Galaxy (image 1) | ✅ Fixed | Funding badge now wraps (auto-height, centered) instead of clipping; verified not clipped at 360px. |

**Verified (headless Chromium, 360 / 390 / 768 / 1440):** 0px horizontal overflow; no roadmap overlap; funding badge not clipped; particles gone; CSS glow present; production build passes.

**Honest limitations (not hidden):**
- I cannot run the **actual** WeChat in-app browser, Samsung Internet, or iOS Safari here. I fixed the documented root causes (font-inflation, WebGL, overflow), which resolve the symptoms in your screenshots — but please re-scan on your real devices after deploy.
- Items **5, 6, 9** are intentionally **not done** pending your input (above).
- CLS: removing WebGL + fixed-size containers should keep CLS ≈ 0; confirm with PageSpeed Insights on the live URL.

---

## 12. v4 changes (confirmed with client)

- **Footer:** removed the Website/globe (🌐) icon per request (it was a circle, item 9, and redundant "link to homepage from the homepage"). Footer now shows **X · Telegram · Email** only.
- **Investor Deck button (item 6):** re-added to the **left** of "REQUEST FULL DECK", uniform style + equal min-width (220px) + centered so both align. Temporary target = email ("Investor Deck Request") to avoid a 404; **swap to a direct download once the public-deck PDF is supplied** (e.g. `/investor-deck-public.pdf`).
- **Aladdin (item 5):** still **not published** (awaiting signed contract + approved wording).
- Everything from v3 (particles→glow, WeChat/Samsung text-size-adjust, roadmap no-overlap, removed demo-video placeholder, brightened dim text, etc.) is included.

**Still pending from client:** public-deck PDF (to switch the Investor Deck button to a download); Aladdin go-ahead.

---

## 13. v5 changes (confirmed with client)

- **Hero:** added **INVESTOR DECK** button to the left of REQUEST FULL DECK (email → "Investor Deck Request"), matching the Download section. Hero CTA order: Investor Deck · Request Full Deck · View Architecture · Contact Us.
- **Team bio:** updated to the balanced founder-led wording — *"V Zheng — Founder & CEO. Led the design and delivery of the platform's MVP, system architecture, and website, directing a lean founding team. Drives technical direction and product strategy at COSMOS Ledger Labs."* (Signals a team / avoids the "solo hero" flag investors dislike, while keeping V Zheng as the driver.)
- **Anchor offset (item 2):** added `scroll-padding-top` (76px desktop / 150px mobile) so jumping to #sections no longer hides the heading behind the fixed nav. Verified heading lands ~105px below the nav.
- Includes all v3/v4 fixes.

**Still pending:** public-deck PDF (to switch both Investor Deck buttons to a direct download); Aladdin go-ahead.

---

## 15. v7 — Hero glow recolored to electric blue (ring reverted)

Reverted to the soft-glow hero (not the ring). Recolored the glow from blue-cyan to electric blue (#2E7BFF family: glowA rgba(46,123,255), glowB rgba(40,110,240)). No particles, no WebGL. Everything else = v5 (hero Investor Deck button, Team A bio, anchor offset, all responsive/WeChat fixes).

---

## 16. v8 — FINAL: original three.js effect restored, tuned + recolored

Client confirmed the **original self-hosted three.js "cosmos" effect** is the desired one. Restored it (from the live Hero.js) and:
- **Finer / softer grid:** gridRes 230 desktop / 150 mobile (was 140/90), voxelSize 0.46 (was 0.2), softer voxel edges (smoothstep ±0.12), lower glow (0.82) & edge glow (0.85) → continuous glow, individual voxels no longer visible.
- **Color:** shader `color_bright` → vec3(0.16,0.50,1.0) ≈ **#2E7BFF electric blue** (matched to client reference). Verified render peak = #2B8DEE.
- **Anti-black-screen:** restored `#cosmos-bg-container { position:absolute; inset:0; z-index:0 }` in globals.css. Verified content stays at top (contentTop≈60, not shoved down) with the canvas injected.
- **three.js dependency** re-added (^0.160.0).
- Kept all confirmed items: hero **INVESTOR DECK** button (left of Request Full Deck), Team A-version bio, anchor scroll-offset, text-size-adjust (WeChat/Samsung), roadmap no-overlap, footer 3 icons (no globe), removed demo-video placeholder, brightened dim text, security headers, SEO, favicon/OG.

**Verified (software WebGL):** effect renders, canvas injected, no black-screen, electric-blue color, build passes.
**Limitation:** rendered via software WebGL here; real GPU output will be smoother/brighter — confirm on device after deploy.

**Still pending from client:** public-deck PDF (to switch both Investor Deck buttons from email to direct download); Aladdin partner section (awaiting signed contract + approved wording).

---

## 18. FINAL — original three.js effect restored verbatim, color-only change

Restored the client's original Hero.js effect EXACTLY (gridRes 90/140, voxelSize 0.2, glowInt 1.2, edgeGlow 1.2, original voxel grid shader, smoothstep ±0.05). The ONLY change to the effect is the shader color: color_bright vec3(0.1,0.4,1.0) → vec3(0.24,0.62,1.0) (electric/neon blue, client-approved). Anti-black-screen rule present; verified canvas injects and content stays at top. Kept confirmed non-effect items (hero Investor Deck button, Team A bio, anchor offset, WeChat/Samsung fixes, roadmap, footer, SEO, headers, etc.).
