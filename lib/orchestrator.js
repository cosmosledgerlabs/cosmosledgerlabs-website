...
async function broadcastUntilConfirmed(connection, raw, signature, lastValidBlockHeight) {
  ...
  for (;;) {
    ...
    // Poll every 2 s for ~8 s, then re-broadcast. Gentle on the public node.
    for (let i = 0; i < 4; i++) {
      await sleep(2000);
...
export function readableError(err) {
  ...
  if (isRateLimited(err)) {
    return 'The public devnet RPC node is rate-limiting this computer (HTTP 429). Wait two minutes and run the flow again, or set a dedicated RPC endpoint.';
  }
  if (isTransient(err)) {
