...
async function sendAndConfirm(web3, connection, provider, tx, pubkey, extraSigners, onNotice) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      // Use a finalized blockhash, not a confirmed one.
      // Helius sits behind a load balancer: a blockhash from one node may be
      // unknown to the next node that handles simulate or send, which surfaces
      // as BlockhashNotFound. A finalized hash is ~32 slots old and known
      // everywhere. Slightly shorter validity window, far more reliable.
