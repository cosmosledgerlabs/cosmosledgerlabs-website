    ...
    try {
      ...
      try {
        ...
        if (sim && sim.value && sim.value.err) {
          console.error('SIMULATE FAILED — err:', JSON.stringify(sim.value.err));
        ...
        if (simErr && String(simErr.message || '').startsWith('Transaction rejected')) {
          throw simErr;
        }
        console.warn('SIMULATE could not run:', simErr);
      }
      ...
      if (extraSigners && extraSigners.length > 0 && provider.signTransaction) {
        console.log('SIGNING PATH: signTransaction + sendRawTransaction, extraSigners =', extraSigners.length);
