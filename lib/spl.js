...
export async function runSetup({ web3, spl, connection, provider, owner }) {
  ...
  const sig0 = await sendAndConfirm(web3, connection, provider, buildTx0, owner);
  ...
  const sigA = await sendAndConfirm(web3, connection, localSigner, buildTxA, payer);
  ...
  const sigB = await sendAndConfirm(web3, connection, localSigner, buildTxB, payer);
